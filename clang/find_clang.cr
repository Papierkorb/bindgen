...
unless clang_binary = find_clang_binary
  print_help_and_exit
end
...
if output.size < 2 # Sanity check
  STDERR.puts %(Unexpected output from "#{OPTIONS[:clang]}": Expected at least two lines.)
  exit 1
end
...
generated_hpp = "#{__DIR__}/include/generated.hpp"
...
makefile_variables = "#{__DIR__}/Makefile.variables"
...
def parse_cli_args
  ...
  OPTIONS[:clang] = "clang++*"
  OPTIONS[:llvm_config] = "llvm-config*"
...
def find_clang_binary : String?
  log %(Searching for binary "#{OPTIONS[:clang]}" in PATH. Minimum version 6.0.0)
...
def find_llvm_config_binary(paths) : String?
  log %(Searching for binary "#{OPTIONS[:llvm_config]}" in PATH. Minimum version 6.0.0)
...
def print_help_and_exit
  ...
  If you have them named differenly than `clang++` and `llvm-config`:
    2) Run the tool with `--clang <name of clang++> --llvm_config <name of llvm_config>`
...
def print_usage_and_exit
  STDERR.puts <<-END
    ...
    Options:
    --clang NAME         Name or pattern of clang binary (default: clang++*)
    --llvm-config NAME   Name or pattern of llvm-config (default: llvm-config*)
